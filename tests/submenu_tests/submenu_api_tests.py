...
class TestSubmenuAPI:
    ...
    async def test_submenu_create_successful(
        ...
        assert (
                create_submenu_response.json().get('title') ==
                create_submenu_data.get('title')
        )
        assert (
                create_submenu_response.json().get('description') ==
                create_submenu_data.get('description')
        )
    ...
    async def test_get_submenu_detail_successful(
        ...
        assert (
                detail_submenu_response.json().get('title') ==
                create_submenu_data.get('title')
        )
        assert (
                detail_submenu_response.json().get('description') ==
                create_submenu_data.get('description')
        )
        assert (
                detail_submenu_response.json().get('menu_id') ==
                response_submenu_data.get('menu_id')
        )
    ...
    async def test_update_detail_of_submenu_successful(
        ...
        assert (
                update_submenu_response.headers.get('content-type') ==
                'application/json'
        )
        assert (
                update_submenu_response.json().get('id') ==
                response_submenu_data.get('id')
        )
        assert (
                update_submenu_response.json().get('title') ==
                update_submenu_data.get('title')
        )
        assert (
                update_submenu_response.json().get('description') ==
                update_submenu_data.get('description')
        )
        assert (
                update_submenu_response.json().get('menu_id') ==
                response_submenu_data.get('menu_id')
        )
