...
class TestMenuAPI:
    ...
    async def test_menu_create_successful(
        ...
        assert (
                create_menu_response.json().get('title') ==
                create_menu_data.get('title')
        )
        assert (
                create_menu_response.json().get('description') ==
                create_menu_data.get('description')
        )
    ...
    async def test_get_menu_detail_successful(
        ...
        assert (
                detail_menu_response.json().get('title') ==
                create_menu_data.get('title')
        )
        assert (
                detail_menu_response.json().get('description') ==
                create_menu_data.get('description')
        )
    ...
    async def test_update_detail_of_menu_successful(
        ...
        assert (
                update_menu_response.headers.get('content-type') ==
                'application/json'
        )
        assert (
                update_menu_response.json().get('id') ==
                response_menu_data.get('id')
        )
        assert (
                update_menu_response.json().get('title') ==
                update_menu_data.get('title')
        )
        assert (
                update_menu_response.json().get('description') ==
                update_menu_data.get('description')
        )
